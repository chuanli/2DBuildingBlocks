%% A script to compute offset statistics for ground truth
% output generators
...
for i_img = 8:8
    ...
    para.defalt_mag = para.thresh_nn; % a default magnitude for assistant generators 


    nameImg = [P.name_path  P.name_dataset  '\' P.name_data '\' P.name_prefix '(' num2str(i_img) ')' P.name_format];
    nameRep = [P.name_path  P.name_dataset  '\' P.name_data '_GT\' P.name_prefix  '(' num2str(i_img) ')_GT.mat'];
    nameOffsetStatisticsGTOutput = [P.name_path  P.name_dataset  '\' P.name_syn '\' P.name_syn_input '\' P.name_prefix  '(' num2str(i_img) ')OffsetStatisticsGT.txt'];
    ...
    fileID = fopen(nameOffsetStatisticsGTOutput,'w');
    fprintf(fileID, '%d \n', size(generators, 2));
