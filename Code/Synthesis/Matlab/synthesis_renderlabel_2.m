...
nameDetection = ['C:\Chuan\data\2DBuildingBlocks\Teaser\Resized\resultAIO\rob\\Teaser(' num2str(sel_id) ')_afmg.mat'];
Detection = [];
...
im = imread(['C:\Chuan\data\2DBuildingBlocks\Teaser\Resized\Teaser(' num2str(sel_id) ').jpg']);
im_out = ones(size(im));
...
%                 v = P.ini_v  + (1 - P.ini_v ) * (v1 + v2)/2;                
                v1 = ((r - y_start + 1) * (x_end - x_start + 1) +  (c - x_start + 1))/((y_end - y_start + 1) * (x_end - x_start + 1));
                v = P.ini_v  + (1 - P.ini_v ) * v1;              
                rgb = hsv2rgb([h, s, v]);
                im_out(r, c, 1) = rgb(1);
                im_out(r, c, 2) = rgb(2);
                im_out(r, c, 3) = rgb(3);
...
% imwrite(im_out, ['Teaser(' num2str(sel_id) ')_label.png']);
