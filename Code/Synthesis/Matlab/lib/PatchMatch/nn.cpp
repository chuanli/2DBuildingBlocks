...
BITMAP *init_dist(Params *p, BITMAP *a, BITMAP *b, BITMAP *ann, BITMAP *bmask, RegionMasks *region_masks, RegionMasks *amask) {
  ...
  if (is_window(p)) {
	...
	else if (p->patch_w == 64) { ans = init_dist_n<64,1,1>(p, a, b, ann, bmask, region_masks, amask); }
	...
	else if (p->patch_w == 64) { ans = init_dist_n<64,1,0>(p, a, b, ann, bmask, region_masks, amask); }
		else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
  } 
	else {
	...
	else if (p->patch_w == 64) { ans = init_dist_n<64,0,0>(p, a, b, ann, bmask, region_masks, amask); }
		else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
  }
  return ans;
}
...
void nn(Params *p, BITMAP *a, BITMAP *b,
  ...
  if (algo == ALGO_CPU) {
    if (is_window(p)) {
	  ...
	  else if (p->patch_w == 64) { nn_n<64,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
    } 
	  ...
	  else if (p->patch_w == 64) { nn_n<64,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
    } 
		else {
	  ...
	  else if (p->patch_w == 64) { nn_n<64,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
    }
  } 
    ...
    if (is_window(p)) {
	  ...
	  else if (p->patch_w == 64) { nn_n_cputiled<64,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
    } 
		else if (bmask == NULL && amask == NULL && region_masks == NULL) {
      ...
      if (p->rs_max == 0) {
		...
		else if (p->patch_w == 64) { nn_n_proponly<64>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles); }
        else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
      } 
			else {
		...
		else if (p->patch_w == 64) { nn_n_cputiled<64,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
        else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
      }
    } 
		else {
	  ...
	  else if (p->patch_w == 64) { nn_n_cputiled<64,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
    }
  } 
    ...
    if (bmask == NULL && amask == NULL && region_masks == NULL) {
	  ...
	  else if (p->patch_w == 64) { nn_n_gpucpu<64,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles); }
      else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
    } 
		else {
	  ...
	  else if (p->patch_w == 64) { nn_n_gpucpu<64,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles); }
      else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
    }
  } 
    ...
    if (bmask == NULL && amask == NULL && region_masks == NULL) {
	  ...
	  else if (p->patch_w == 64) { nn_n_fullrand<64,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles); }
      else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
    } 
		else {
	  ...
	  else if (p->patch_w == 64) { nn_n_fullrand<64,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles); }
      else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
    }
  } 
	else {
  ...
  if (p->vote_algo == VOTE_MEAN) {
    if (p->algo == ALGO_CPUTILED) {
      ...
      if (bnn == NULL && bmask == NULL && bweight == NULL && amask == NULL && aweight == NULL && region_masks == NULL) {
		...
		else if (p->patch_w == 64) { ans = vote_n_cputiled<64,int,1,0>(p, b, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, region_masks, aconstraint, mask_self_only); }
        else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
		...
		else if (p->patch_w == 64) { ans = vote_n_cputiled<64,int,0,1>(p, b, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, region_masks, aconstraint, mask_self_only); }
        else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
      } else {
		...
		else if (p->patch_w == 64) { ans = vote_n_cputiled<64,double,0,0>(p, b, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, region_masks, aconstraint, mask_self_only); }
        else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
      }
    } else {
      if (p->cores == 1) {
		...
		else if (p->patch_w == 64) { ans = vote_n<64>(p, b, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, region_masks, aconstraint, mask_self_only); }
        else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
      } else {
		...
		else if (p->patch_w == 64) { ans = vote_n_openmp<64>(p, b, ann, bnn, bmask, bweight, coherence_weight, complete_weight, amask, aweight, ainit, region_masks, aconstraint, mask_self_only); }
        else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
      }
    }
  } else {
...
void minnn(Params *p, BITMAP *a, BITMAP *b, BITMAP *ann, BITMAP *annd, BITMAP *ann_prev, BITMAP *bmask, int level, int em_iter, RecomposeParams *rp, RegionMasks *region_masks, RegionMasks *amask, int ntiles) {
  if (is_window(p)) {
    if      (p->patch_w == 1 ) { return minnn_n<1 , 1, 1>(p, a, b, ann, annd, ann_prev, bmask, level, em_iter, rp, region_masks, amask, ntiles); }
		...
		else if (p->patch_w == 64) { return minnn_n<64, 1, 1>(p, a, b, ann, annd, ann_prev, bmask, level, em_iter, rp, region_masks, amask, ntiles); }
		else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
  } 
    ...
    if      (p->patch_w == 1 ) { return minnn_n<1 , 0, 1>(p, a, b, ann, annd, ann_prev, bmask, level, em_iter, rp, region_masks, amask, ntiles); }
		...
		else if (p->patch_w == 64) { return minnn_n<64, 0, 1>(p, a, b, ann, annd, ann_prev, bmask, level, em_iter, rp, region_masks, amask, ntiles); }
		else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
  } 
	else {
    if      (p->patch_w == 1 ) { return minnn_n<1 , 0, 0>(p, a, b, ann, annd, ann_prev, bmask, level, em_iter, rp, region_masks, amask, ntiles); }
		...
		else if (p->patch_w == 64) { return minnn_n<64, 0, 0>(p, a, b, ann, annd, ann_prev, bmask, level, em_iter, rp, region_masks, amask, ntiles); }
		else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
  }
}
