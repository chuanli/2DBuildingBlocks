function A_rec_gray = func_rec_gray(A_rec_gray, A_num_rows, A_num_cols, h, w, B_patches, match_A2B,  match_B2A)
...
for i = 1:size(match_B2A, 1)
    ...
    new_A_rec_gray(A_row:A_row + h - 1, A_col:A_col + w - 1) = new_A_rec_gray(A_row:A_row + h - 1, A_col:A_col + w - 1) + reshape(B_patches(i, :), h, w);
    new_A_rec_count(A_row:A_row + h - 1, A_col:A_col + w - 1) = new_A_rec_count(A_row:A_row + h - 1, A_col:A_col + w - 1) + 1;
end
